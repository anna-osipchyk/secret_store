...
urlpatterns = [
    ...
    url(r'^(?P<fk>\d+)/variables/(?P<pk>\d+)/$', ProjectVariables.as_view({'get': 'retrieve'})),
    url(r'^(?P<fk>\d+)/variables/(?P<pk>\d+)/$', ProjectVariables.as_view({'patch': 'update'})),
    url(r'^(?P<fk>\d+)/variables/(?P<pk>\d+)/$', ProjectVariables.as_view({'delete': 'destroy'}))
